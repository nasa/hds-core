...
_Sources: Issue #40 (C-30/C-40/C-50/C-60/C-70 pairs — all manually verified via WebAIM). Issue #39 (White and R-Tint on B-Shade)._

### Known gap: error text on blue palette (Issue #39)

`R-Tint` (`#ff5c52`) on `B-Shade` (`#0b3d91`) = **3.3:1**. This passes the 3:1 non-text threshold (⚠️), but error messages are normal text — they require 4.5:1 AA. The current token assignment fails that threshold.

Affects all form error messages inside a `.hds-palette-blue` wrapper.

Proposed fix (pending Creative Director review): use `White` for blue palette error text (`#ffffff` on `#0b3d91` = 9.9:1 AAA). The error border can remain at `R-Tint`.

Tracked in [Issue #39](https://github.com/nasa/hds-core/issues/39).
