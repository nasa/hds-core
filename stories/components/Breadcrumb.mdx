...
<Note type="uswds">USWDS 3.14.0 made breadcrumb wrapping the default and moved single-line truncation behind a new `usa-breadcrumb--truncate` class (`usa-breadcrumb--wrap` is now an inert no-op upstream). HDS does not use either: the 3-element ellipsis pattern above is authored in the markup, not produced by a modifier. Avoid `usa-breadcrumb--truncate` — it clips the trail with `overflow: hidden`, which also clips the HDS focus ring.</Note>
