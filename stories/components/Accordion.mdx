...
<Note type="uswds">USWDS 3.14.0 moved the expand/collapse icon to the leading (left) edge by default. HDS keeps the chevron on the trailing edge, per the Figma spec, by setting `$theme-accordion-icon-position: 'end'` in the theme. The USWDS `usa-accordion--icon-start` and `usa-accordion--icon-end` modifier classes are **not supported** in HDS — they reserve padding for an icon HDS does not draw, which leaves the heading label indented with the chevron still on the right.</Note>
