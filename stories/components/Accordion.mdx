...
<Note type="uswds">USWDS 3.14.0 moved the icon to the leading (left) edge by default; HDS keeps the chevron on the trailing edge. The USWDS `usa-accordion--icon-start` and `usa-accordion--icon-end` modifier classes are not currently supported in HDS Core.</Note>
