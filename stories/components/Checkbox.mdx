...
Checkboxes are a form element that allows users to select zero, one, or multiple options from a list. HDS checkboxes use standard USWDS [`.usa-checkbox`](https://designsystem.digital.gov/components/checkbox/) markup with no additional classes.
...
When a required checkbox group has no selection, display an error message below the options.
...
- **Mutually exclusive choices** — use [radio buttons](?path=/docs/components-radio-button-guidance--docs) when only one option can be selected (e.g., Yes or No).
