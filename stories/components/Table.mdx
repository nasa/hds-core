...
<Note type="uswds">
  <p>USWDS stacked table variants (`.usa-table--stacked` and `.usa-table--stacked-header`) for mobile layouts are not yet tested with HDS styling. Use the scrollable variant for mobile-friendly tables.</p>
</Note>
...
<Note type="figma">
  <p>The HDS Figma spec includes advanced table features (filter panel, search, download, print actions, mobile accordion layout) and does not define a striped variant. The USWDS `.usa-table--striped` class works but is not HDS-specified. Advanced features and striped styling are planned for future review.</p>
</Note>
