...
<Canvas of={ProseStories.DefaultEmbed} />
...
- **Individual components.** If you control the markup, use component classes directly — [List](?path=/docs/components-list--guidance) (`.usa-list`), [Table](?path=/docs/components-table--guidance) (`.usa-table`), [Link](?path=/docs/components-link--guidance) (`.usa-link`). Component classes give more precise control and don't require a wrapper.
...
- **Lists inside prose use bare-element styling.** For ordered lists with precise DM Mono numeral alignment, use `<ol class="usa-list" role="list">` instead. See [List](?path=/docs/components-list--guidance) for details.
