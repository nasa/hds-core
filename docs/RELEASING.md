...
- **`publish`** — runs **only** when a bump just landed. It builds, publishes to npm over OIDC, tags `vX.Y.Z`, and creates a GitHub Release with the dist zip, its SHA-256 checksum, and the SBOM attached. This job runs in the **`npm-publish`** environment.

Each release carries three assets: `hds-core-vX.Y.Z-dist.zip`, `hds-core-vX.Y.Z-dist.zip.sha256`, and `sbom.json`. The same SHA-256 is also written into the release body under **Verify your download**, so adopters who self-host the zip can check it either way — see the [No-Build Environments](https://nasa.github.io/hds-core/?path=/docs/guides-no-build-environments--docs) guide.
