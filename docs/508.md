...
- **Motion**: Interaction feedback only, 500ms or less, never carrying information. One `prefers-reduced-motion: reduce` block removes all of it. See DESIGN.md § Motion for the full rules on what is and is not animated.
...
| 2.2.2 | Pause, Stop, Hide | Supports | No HDS Core component moves, blinks, scrolls, or updates on its own. The motion HDS Core ships is entirely interaction feedback — a control responding to hover or activation — which starts only when the user acts and finishes in 500ms or less. Looping and attention-seeking animation is prohibited by the motion token descriptions in `tokens.json`. Consuming applications that build longer or automatic motion on the HDS motion tokens are responsible for their own pause mechanism. |
| 2.3.1 | Three Flashes or Below Threshold | Supports | No HDS Core component flashes. Each interactive element transitions at most once per user action, well under three times per second. The Data Visualization guidance carries the same limit for chart animation built by consuming applications. |
...
Level AAA is not part of the required scope. This one criterion is recorded because HDS Core meets it by design and adopters ask about it.

| # | Criterion | Status | Remarks |
| --- | --- | --- | --- |
| 2.3.3 | Animation from Interactions | Supports | HDS Core animates hover and activation states on a small set of components (buttons, icon buttons, navigation links, pagination, breadcrumb, accordion chevron, sortable table headers). All of it is non-essential: it changes color, shadow, or filter, and never conveys information on its own. A single `prefers-reduced-motion: reduce` block in `base/_reduced-motion.scss` sets all four `--hds-motion-duration-*` custom properties to `0ms`, which removes every HDS transition at once because components read their durations from those properties. Verified in headless Chromium with the reduced-motion preference emulated. |
