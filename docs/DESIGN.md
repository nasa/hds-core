...
**Glyph size.** Every glyph is normalized to a 20-unit longest side and nothing more; there is no per-mark correction for perceived weight. Social glyphs then render at 70% of the button rather than the 60% used for UI glyphs, since brand marks are denser and read undersized at 60%. That puts the widest marks at roughly 82% of the circle's diameter. The residual size variation across the set is aspect ratio alone — a 2.2:1 mark cannot fill a circle the way a 1:1 mark does — and is left alone.
