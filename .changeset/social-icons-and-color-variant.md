...
**Three platforms ship a glyph but no brand colour.** Spotify, LinkedIn, and Apple Podcasts publish marks that already contain their own container with the glyph knocked out, so compositing them onto a coloured circle inverts the lockup and doubles the container — and their brand guidelines forbid separating mark from container. They fall back to HDS palette colours in the colour variant, exactly like `rss` and `mail`; their black-and-white rendering is each platform's official monochrome logo and is correct. See the Icon Button docs for "Bring your own logo" if you need full-colour artwork for these.

**Generic (non-platform) share targets.** `share`, `notification` (bell), `rss`, and `mail` cover the non-platform buttons in a share row. The HDS Figma spec places these on the NASA Blue **secondary** circle rather than the social circle, since they are on-page actions rather than links out to a platform. `rss` is the exception with a colour of its own — `data-hds-social="rss"` supplies the de-facto feed orange, which has no trademark owner. `share`, `notification`, and `mail` stay on HDS palette colours.
