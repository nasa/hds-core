...
These components have no HDS theming yet, and ship as bare USWDS defaults inside `@layer uswds`. Currently, the sole HDS treatment for these is in `base/_palettes.scss`: `.usa-identifier` is pinned to the black palette so its links stay legible regardless of the surrounding site palette.
